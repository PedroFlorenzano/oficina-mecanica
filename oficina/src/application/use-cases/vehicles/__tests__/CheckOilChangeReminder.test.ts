...
describe("CheckOilChangeReminder", () => {
  ...
  it("deve retornar null quando km atual é menor que lastChange + 4000", async () => {
    // 40000 + (10000 - 4000) = 46000, km atual: 43000 — abaixo do limiar
  ...
  it("deve retornar alerta com overdue=false entre (interval-alertWindow) e interval km após troca", async () => {
    // 40000 + 10000 - 4000 = 46000 (início do alerta), 40000 + 10000 = 50000 (limite), km: 47000
    const vehicleRepo = makeVehicleRepo(makeVehicle(47000));
    const orderRepo = makeOrderRepo(lastOilOrder);
    const useCase = new CheckOilChangeReminder(vehicleRepo, orderRepo);
    ...
    expect(result!.dueAt).toBe(50000);
    expect(result!.lastChangeMileage).toBe(40000);
    expect(result!.currentMileage).toBe(47000);
  });

  it("deve retornar alerta com overdue=true a partir de 10000 km após troca", async () => {
    // km atual: 51000 >= 40000 + 10000 = 50000
    const vehicleRepo = makeVehicleRepo(makeVehicle(51000));
    const orderRepo = makeOrderRepo(lastOilOrder);
    const useCase = new CheckOilChangeReminder(vehicleRepo, orderRepo);
  ...
  it("deve retornar alerta com overdue=false exatamente em lastChange + (interval - alertWindow)", async () => {
    // 40000 + (10000 - 4000) = 46000
  ...
  it("deve retornar alerta com overdue=true exatamente em lastChange + 10000", async () => {
    const vehicleRepo = makeVehicleRepo(makeVehicle(50000));
    const orderRepo = makeOrderRepo(lastOilOrder);
    const useCase = new CheckOilChangeReminder(vehicleRepo, orderRepo);
